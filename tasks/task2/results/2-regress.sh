...
timeout 2 bash -c "</dev/tcp/${DB_HOST}/${DB_PORT_BOOKINGS}" \
  || { echo "❌ Не удалось подключиться к ${DB_HOST}:${DB_PORT_BOOKINGS}"; exit 1; }

# Загрузка фикстур
echo "🧪 Загрузка фикстур...1"
PGPASSWORD="${DB_PASSWORD}" psql -h "${DB_HOST}" -p "${DB_PORT}" -U "${DB_USER}" "${DB_NAME}" < init-fixtures.sql
echo "🧪 Загрузка фикстур...2"
PGPASSWORD="${DB_PASSWORD}" psql -h "${DB_HOST}" -p "${DB_PORT_BOOKINGS}" -U "${DB_USER}" "${DB_NAME}" < init-fixtures-bookings.sql 
